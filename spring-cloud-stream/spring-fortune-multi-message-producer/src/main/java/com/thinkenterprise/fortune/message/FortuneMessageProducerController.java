...
public class FortuneMessageProducerController {
	...
	@RequestMapping("/sendJsonRandom")
	public String sendJsonFortune() {
		
		Iterable<Fortune> result = fortuneRepository.randomFortunes();
						
		if(fortuneOutputStream.fortuneOutput().send(MessageBuilder.withPayload(result.iterator().next()).setHeader("content-type", "application/json").build()))
	...
	@RequestMapping("/sendStringRandom")
	public String sendStringFortune() {
		
		Iterable<Fortune> result = fortuneRepository.randomFortunes();
		String fortuneText = result.iterator().next().getText();
		
		if(fortuneOutputStream.fortuneOutput().send(MessageBuilder.withPayload(fortuneText).setHeader("content-type", "text/plain").build()))
			return "send message with fortune string" ; 
		else			
			return "send message with fortune string error";  
